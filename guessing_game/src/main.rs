...
fn main() {
   ...
   loop {
	...
	.expect("Failed to read line");
      ...
      // we already have a variable 'guess'
      // Rust allows us to shadow the previous value with a new one
      // useful for when we want to convet a value from one type to another (annotate and assign with : u32)
      // trim removes whitespace and newlines, read_line ends with \n
      // parse method on strings converts a string to another type
      // now that we have assigned guess to u32, Rust will infer that secret_number should be u32
